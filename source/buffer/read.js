...
module.exports = function(length) {
  ...
  return serialize(this, () => new Promise((resolve, reject) => {
    ...
    if (0 <= i && j <= n) return resolve(this._buffer.slice(i, this._offset = j));
    ...
    else if (n && n < this._size) return resolve(this._buffer.slice(i, this._offset = n));
    ...
    // Is the starting part of the range buffered? Combine with the next chunk.
      ...
      return this._source.read(this._size).then(
        (buffer) => resolve(Buffer.concat([this._buffer.slice(i), (this._buffer = buffer).slice(0, this._offset = j - n)])),
        (error) => reject(error)
      );
    }
    ...
    return this._source.seek(this._position = start).read(this._size).then(
      (buffer) => resolve((this._buffer = buffer).slice(0, this._offset = length)),
      (error) => reject(error)
    );
  }));
};
